...
function resolveClientCapabilities(params: {
  ...
  const baseCapabilities: ClientCapabilities = {
    ...
    // tacp elicitation seam: advertise form mode so agents enable structured questions
    elicitation: {
      form: {},
    },
  };
...
export class AcpClient {
  ...
  private async handleAskUserQuestion(
    ...
    if (!this.options.onAskUserQuestion) {
      throw RequestError.methodNotFound("_x.ai/ask_user_question");
    }
    const sessionId =
      typeof params.sessionId === "string" ? params.sessionId : undefined;
    const signal = sessionId
      ? this.cancellationSignalForSession(sessionId)
      : new AbortController().signal;
    try {
      return await this.options.onAskUserQuestion(
        { sessionId, raw: params },
        { signal },
      );
    } catch (error) {
      this.log(
        `onAskUserQuestion threw: ${
  ...
  private async handleElicitationRequest(
    ...
    const signal = sessionId
      ...
      : new AbortController().signal;
    try {
      const decision = await this.options.onElicitationRequest(
        { sessionId: sessionId || undefined, raw: params },
        { signal },
      );
      if (!decision) {
        return { action: "decline" };
      ...
      return { action: decision.action } as CreateElicitationResponse;
    } catch (error) {
      this.log(
        `onElicitationRequest threw, declining: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
      return { action: "decline" };
