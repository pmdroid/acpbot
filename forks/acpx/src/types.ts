...
export type AcpClientOptions = {
  ...
  /**
   * Host hook for elicitation/create. Awaited unbounded — same contract as
   * onPermissionRequest. Returning undefined declines.
   */
  ...
  /**
   * Grok Build multi-choice questions via ext method `_x.ai/ask_user_question`.
   * Awaited unbounded — same contract as onPermissionRequest / onElicitationRequest.
   */
  onAskUserQuestion?: (
    req: AcpAskUserQuestionRequest,
    ctx: { signal: AbortSignal },
  ) => Promise<AcpAskUserQuestionResponse>;
};

export type AcpAskUserQuestionRequest = {
  sessionId?: string;
  raw: unknown;
};

/** Opaque JSON response returned to the Grok agent tool. */
export type AcpAskUserQuestionResponse = Record<string, unknown>;
