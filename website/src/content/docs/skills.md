---
title: Skills
description: Bundled telegram, schedules, multi-agent, memory skills and global install.
order: 15
section: reference
---
...
| **memory** | [`skills/memory/SKILL.md`](https://github.com/pmdroid/acpbot/blob/main/skills/memory/SKILL.md) | Repo-local durable notes via `memory_read` / `memory_write` (`MEMORY.md`, daily) |
...
- `~/.agents/skills/{telegram,schedules,multi-agent,memory}`
- `~/.grok/skills/{telegram,schedules,multi-agent,memory}`
- `~/.claude/skills/{telegram,schedules,multi-agent,memory}`
